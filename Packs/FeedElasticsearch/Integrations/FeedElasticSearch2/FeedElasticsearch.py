...
def push_indicators_command(client):
    """Implements es-push-indicators command"""
    indicators_to_push = []
    indicators = json.loads(demisto.args().get('indicators'))
    index = demisto.args().get('index')

    for indicator in indicators:
        if indicator.get('Value') and indicator.get('Type'):
            indicators_to_push.append({
                "_index": index,
                "_id": indicator.get('Value'),
                "value": indicator.get('Value','None'),
                "type": indicator.get('Type')
            })

    raw_response = helpers.bulk(client.es, indicators_to_push, index=index)
    return_results(f'{list(raw_response)[0]} Indicators Pushed Successfully')
...
def main():
    try:
        ...
        elif demisto.command() == 'es-push-indicators':
            push_indicators_command(client)
